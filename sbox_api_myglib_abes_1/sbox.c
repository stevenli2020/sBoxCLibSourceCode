...
int sbox_init(char* token, char** pp, char** msk){
    ...
    cca_odabe_msk_t* rmsk;
    cca_odabe_pub_t* rpub;
    res = cca_odabe_setup_1(group->pairing, group->pairingDesc, &rpub, &rmsk);
    if(res){
        *pp = encode64_str(cca_odabe_pub_serialize(rpub));
        *msk = encode64_str(cca_odabe_msk_serialize(rmsk));
    }
    cca_odabe_msk_free(rmsk);
    cca_odabe_pub_free(rpub);
    return res;
    
}

char* sbox_genABEUUID(char* pp){
    cca_odabe_pub_t* pub = cca_odabe_pub_unserialize(decode64_str(pp), 1);
...
int sbox_gen_uks(char* pp, char* imsk, char** attrs, int len, char** tk_s, char** dk_s){
    int res = 0;
    cca_odabe_pub_t* pub = cca_odabe_pub_unserialize(decode64_str(pp), 1);
    cca_odabe_msk_t* msk = cca_odabe_msk_unserialize(pub->p, decode64_str(imsk), 1);
    cca_odabe_tk_t* tk;
    cca_odabe_dk_t* dk;
    res = cca_odabe_keygen(pub, msk, attrs, &tk, &dk);
    if(res){
        *tk_s = encode64_str(cca_odabe_tk_serialize(tk));
        *dk_s = encode64_str(cca_odabe_dk_serialize(dk));
        cca_odabe_tk_free(tk);
        cca_odabe_dk_free(dk);
    }
    cca_odabe_msk_free(msk);
    cca_odabe_pub_free(pub);
    return res;
}
...
int sbox_transform(char* pp, char* itk, char* abec, char** abetc){
    int res = 0;
    cca_odabe_pub_t* pub = cca_odabe_pub_unserialize(decode64_str(pp), 1);
    cca_odabe_tk_t* tk = cca_odabe_tk_unserialize(pub->p, decode64_str(itk), 1);
    cca_odabe_cph_t* cph = cca_odabe_cph_unserialize(pub->p, decode64_str(abec), 1);
    cca_odabe_tcph_t* trans;
    
    res = cca_odabe_trans(pub, tk, cph, &trans);
    if(res){
        *abetc = encode64_str(cca_odabe_tcph_serialize(trans));
        cca_odabe_tcph_free(trans);
    }
    cca_odabe_cph_free(cph);
    cca_odabe_tk_free(tk);
    cca_odabe_pub_free(pub);
    return res;
}
...
int sbox_enc(char* pp, char* policy, char** abec, unsigned char** m_b){
    ...
    if(res){
        *abec = encode64_str(cca_odabe_cph_serialize(cph));
...
int sbox_dec(char* pp, char* idk, char* abetc, unsigned char** m_b){
    int res = 0;
    cca_odabe_pub_t* pub = cca_odabe_pub_unserialize(decode64_str(pp), 1);
    cca_odabe_dk_t* dk = cca_odabe_dk_unserialize(pub->p, decode64_str(idk), 1);
    cca_odabe_tcph_t* tcph = cca_odabe_tcph_unserialize(pub->p, decode64_str(abetc), 1);

    element_t m;
    res = cca_odabe_dec(pub, dk, tcph, m);
    if(res){
        res = element_length_in_bytes(m);
        *m_b = malloc(res);
        element_to_bytes(*m_b, m);
...
int sbox_re_enc(char* pp, char* idk, char* abetc, char* npolicy, char** abec){
    ...
    res = cca_odabe_dec(pub, dk, tcph, m);
    if(res){
        res = cca_odabe_enc(pub, m, npolicy, &cph);
        if(res){
            *abec = encode64_str(cca_odabe_cph_serialize(cph));
            cca_odabe_cph_free(cph);
        }
